...
module Api
  module V1
    class UsersViewingPartyController < ApplicationController
      rescue_from ArgumentError, with: :invalid_parameters
      def create
        UserViewingPartyValidator.new(params).validate_all

        UserViewingParty.create!(viewing_party_id: ViewingParty.find(params[:party_id]).id,
                                 user_id: User.find(params[:invitees_user_id]).id, host: false)
        ...
        render json: ViewingPartySerializer.new(ViewingParty.find(params[:party_id]),
                                                { params: { invitees: invitees } }), status: :created
      end
