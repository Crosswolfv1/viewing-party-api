...
class ViewingPartyValidator
  ...
  def validate_invitees
    raise ArgumentError, "Validation failed: Invitees can't be blank" unless @params.key?(:invitees)
  ...
  def validate_start_end_time
    if @params[:start_time].nil? || @params[:end_time].nil?
      raise ArgumentError,
            'Invalid or missing start_time or end_time'
    end
  ...
  def validate_session_length
    raise ArgumentError, "Validation failed: Movie can't be blank" if @params[:movie_id].blank?
